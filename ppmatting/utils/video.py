...
class VideoReader(paddle.io.Dataset):
    ...
    def __init__(self, path, transforms=None):
        ...
        if not os.path.exists(path):
            raise IOError("There is not found about video path:{} ".format(path))
    ...
    def __getitem__(self, idx):
        ...
        data["ori_img"] = frame.transpose((2, 0, 1)) / 255.0
...
class VideoWriter:
    ...
    def write(self, frames):
        ...
        if isinstance(frames, paddle.Tensor):
            if frames.ndim != 4:
                raise ValueError(
                    "The frames should have the shape like [N, C, H, W], but it is {}".format(
                        frames.shape
                    )
                )
            ...
            if not (c == 1 or c == 3):
                raise ValueError(
                    "the channels of frames should be 1 or 3, but it is {}".format(c)
                )
