...
from ppmatting.utils import estimate_foreground_ml, mkdir
...
def predict(
    ...
    if nranks > 1:
        ...
        trimap_lists = (
            partition_list(trimap_list, nranks) if trimap_list is not None else None
        )
    else:
    ...
    with paddle.no_grad():
        for i, im_path in enumerate(img_lists[local_rank]):
            ...
            trimap = trimap_lists[local_rank][i] if trimap_list is not None else None
