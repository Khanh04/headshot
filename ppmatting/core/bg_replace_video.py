...
def build_loader_writter(video_path, transforms, save_dir):
    ...
    writer = VideoWriter(
        save_path, reader.fps, frame_size=(reader.width, reader.height), is_color=True
    )
...
def bg_replace_video(
    ...
    loader, writer = build_loader_writter(video_path, transforms, save_dir=save_dir)
    # Get bg
    bg_reader = get_bg(bg_path, shape=(loader.dataset.height, loader.dataset.width))
