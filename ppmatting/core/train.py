        ...
        for data in loader:
            ...
            if precision == "fp16":
                with paddle.amp.auto_cast(
                    ...
                    logit_dict, loss_dict = (
                        ddp_model(data) if nranks > 1 else model(data)
                    )
                ...
                logit_dict, loss_dict = ddp_model(data) if nranks > 1 else model(data)
            ...
            if (iter) % log_iters == 0 and local_rank == 0:
                ...
                for key, value in avg_loss.items():
                    if key != "all":
                        loss_str = loss_str + " " + key + "={:.4f}".format(value)
                logger.info(
                ...
                if use_vdl:
                    ...
                    log_writer.add_scalar(
                        "Train/batch_cost", avg_train_batch_cost, iter
                    )
                    log_writer.add_scalar(
                        "Train/reader_cost", avg_train_reader_cost, iter
                    )
                    if iter % log_image_iters == 0:
                        ...
                        for key in data["gt_fields"]:
                            key = key[0]
                            vis_dict["/".join(["ground truth", key])] = data[key][0]
                        # predict
                        for key, value in logit_dict.items():
                            vis_dict["/".join(["predict", key])] = logit_dict[key][0]
                        visual_in_traning(
            ...
            if (iter % save_interval == 0 or iter == iters) and local_rank == 0:
                if val_dataset is not None and iter >= eval_begin_iters:
                    ...
                    if use_vdl:
                        for key, value in metrics_data.items():
                            log_writer.add_scalar("Evaluate/" + key, value, iter)
