...
class Resize:
    ...
    def __call__(self, data):
        ...
        for key in data.get("gt_fields", []):
            if key == "trimap":
                data[key] = functional.resize(
                    data[key], self.target_size, cv2.INTER_NEAREST
                )
            else:
                data[key] = functional.resize(data[key], self.target_size, interp)
        return data
...
class RandomResize:
    ...
    def __init__(self, size=None, scale=None):
        if isinstance(size, list) or isinstance(size, tuple):
            if len(size) != 2:
                raise ValueError(
                    "`size` should include 2 elements, but it is {}".format(size)
                )
        elif size is not None:
            raise TypeError(
                "Type of `size` is invalid. It should be list or tuple, but it is {}".format(
                    type(size)
                )
            )
        ...
        if scale is not None:
            if isinstance(scale, list) or isinstance(scale, tuple):
                if len(scale) != 2:
                    raise ValueError(
                        "`scale` should include 2 elements, but it is {}".format(scale)
                    )
            else:
                raise TypeError(
                    "Type of `scale` is invalid. It should be list or tuple, but it is {}".format(
                        type(scale)
                    )
                )
        self.size = size
...
class ResizeToIntMult:
    ...
    def __call__(self, data):
        ...
        for key in data.get("gt_fields", []):
            if key == "trimap":
                data[key] = functional.resize(data[key], (rw, rh), cv2.INTER_NEAREST)
            else:
...
class Normalize:
    ...
    def __init__(self, mean=(0.5, 0.5, 0.5), std=(0.5, 0.5, 0.5)):
        ...
        self.std = std
        if not (
            isinstance(self.mean, (list, tuple)) and isinstance(self.std, (list, tuple))
        ):
            raise ValueError(
                "{}: input type is invalid. It should be list or tuple".format(self)
            )
...
class RandomCropByAlpha:
    ...
    def __init__(self, crop_size=((320, 320), (480, 480), (640, 640)), prob=0.5):
    ...
    def __call__(self, data):
        ...
        if target != long_edge:
            ...
            for key in data.get("gt_fields", []):
                if key == "trimap":
                    data[key] = functional.resize_long(
                        data[key], target, cv2.INTER_NEAREST
                    )
                else:
    ...
    def __init__(self, max_short=None, min_short=None):
        if max_short is not None:
            if not isinstance(max_short, int):
                raise TypeError(
                    "Type of `max_short` is invalid. It should be int, but it is {}".format(
                        type(max_short)
                    )
                )
        if min_short is not None:
            if not isinstance(min_short, int):
                raise TypeError(
                    "Type of `min_short` is invalid. It should be int, but it is {}".format(
                        type(min_short)
                    )
                )
        if (max_short is not None) and (min_short is not None):
            if min_short > max_short:
                raise ValueError(
                    "`max_short should not smaller than min_short, but they are {} and {}".format(
                        max_short, min_short
                    )
                )
        ...
        if n > 0:
            if np.random.randint(0, n) == 0:
                ...
                for key in data.get("gt_fields", []):
                    if key == "trimap":
                        continue
                    data[key] = cv2.GaussianBlur(data[key], (radius, radius), 0, 0)
        return data
