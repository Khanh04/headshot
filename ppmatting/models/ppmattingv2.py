...
class PPMattingV2(nn.Layer):
    ...
    def __init__(
        ...
        self.dpp = DoublePyramidPoolModule(
            stride=2,
            input_channel=sum(self.backbone_channels[i] for i in self.dpp_index),
        ...
        self.matting_head_mlff8x = MattingHead(in_chan=decoder_channels[2], mid_chan=32)
    ...
    def forward(self, inputs):
        ...
        x = self.mlff32x(input_32x, paddle.shape(feats_backbone[-1])[-2:])  # 32x

        input_16x = [feats_backbone[-2], x, dpp_out]
        x = self.mlff16x(input_16x, paddle.shape(feats_backbone[-2])[-2:])  # 16x
        ...
        input_4x.append(F.interpolate(img, feats_backbone[-4].shape[2:], mode="area"))
        x = self.mlff4x(input_4x, paddle.shape(feats_backbone[-4])[-2:])  # 4x

        input_2x = [feats_backbone[-5], x]
        input_2x.append(F.interpolate(img, feats_backbone[-5].shape[2:], mode="area"))
        x = self.mlff2x(input_2x, paddle.shape(feats_backbone[-5])[-2:])  # 2x

        x = F.interpolate(x, input_shape[-2:], mode="bilinear", align_corners=False)
...
class MattingHead(nn.Layer):
    def __init__(self, in_chan, mid_chan, mid_num=1, out_channels=1):
        ...
        self.conv = layers.ConvBNReLU(
            in_chan, mid_chan, kernel_size=3, stride=1, padding=1, bias_attr=False
        )
...
class DoublePyramidPoolModule(nn.Layer):
    ...
    def __init__(
        ...
        if isinstance(len_trans, int):
            ...
            raise ValueError("`len_trans` only support int, list and tuple type")
        ...
        if self.merge_type == "concat":
            ...
            in_chan = mid_channel
        self.conv_out = layers.ConvBNReLU(in_chan, output_channel, kernel_size=1)
...
class Conv2DBN(nn.Layer):
    def __init__(
        ...
        bn_weight_attr = paddle.ParamAttr(
            initializer=nn.initializer.Constant(bn_weight_init), learning_rate=lr_mult
        )
...
class Attention(nn.Layer):
    def __init__(
        self, dim, key_dim, num_heads, attn_ratio=4, activation=None, lr_mult=1.0
    ):
        super().__init__()
        ...
        self.proj = nn.Sequential(
            activation(), Conv2DBN(self.dh, dim, bn_weight_init=0, lr_mult=lr_mult)
        )
    ...
    def forward(self, x):
        ...
        qq = (
            self.to_q(x)
            .reshape([0, self.num_heads, self.key_dim, -1])
            .transpose([0, 1, 3, 2])
        )
        kk = self.to_k(x).reshape([0, self.num_heads, self.key_dim, -1])
        vv = (
            self.to_v(x)
            .reshape([0, self.num_heads, self.d, -1])
            .transpose([0, 1, 3, 2])
        )
