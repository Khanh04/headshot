...
class GradientLoss(nn.Layer):
    ...
    def sobel(self, input):
        ...
        if not len(input.shape) == 4:
            raise ValueError(
                "Invalid input shape, we expect NCHW, but it is ", input.shape
            )
...
class LaplacianLoss(nn.Layer):
    ...
    def __init__(self):
        super().__init__()
        self.gauss_kernel = self.build_gauss_kernel(size=5, sigma=1.0, n_channels=1)
