...
class RVM(nn.Layer):
    ...
    def __init__(
        ...
        rd_fea_channels = self.backbone.feat_channels[:-1] + [lraspp_out_channels]
    ...
    def forward_(
        ...
        if not segmentation_pass:
            ...
            fgr = fgr.clip(0.0, 1.0)
            pha = pha.clip(0.0, 1.0)
            return [fgr, pha, *rec]
        else:
...
class OutputBlock(nn.Layer):
    def __init__(self, in_channels, src_channels, out_channels):
        ...
        self.conv = nn.Sequential(
            nn.Conv2D(
                in_channels + src_channels, out_channels, 3, 1, 1, bias_attr=False
            ),
            nn.BatchNorm2D(out_channels),
            nn.ReLU(),
...
class FastGuidedFilterRefiner(nn.Layer):
    ...
    def forward_single_frame(self, fine_src, base_src, base_fgr, base_pha):
        ...
        fgr, pha = self.guilded_filter(
            ...
            paddle.concat([fine_src, fine_src_gray], axis=1),
        ).split([3, 1], axis=1)
...
class DeepGuidedFilterRefiner(nn.Layer):
    def __init__(self, hid_channels=16):
        ...
        self.conv = nn.Sequential(
            nn.Conv2D(
                4 * 2 + hid_channels, hid_channels, kernel_size=1, bias_attr=False
            ),
    ...
    def forward_single_frame(self, fine_src, base_src, base_fgr, base_pha, base_hid):
        fine_x = paddle.concat([fine_src, fine_src.mean(1, keepdim=True)], axis=1)
