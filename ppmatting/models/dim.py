...
class DIM(nn.Layer):
    ...
    def __init__(self, backbone, stage=3, decoder_input_channels=512, pretrained=None):
        super().__init__()
    ...
    def loss(self, logit_dict, label_dict, loss_func_dict=None):
        ...
        if self.stage == 1 or self.stage == 3:
            comp_pred = (
                logit_dict["alpha_raw"] * label_dict["fg"]
...
class Up(nn.Layer):
    def __init__(self, input_channels, output_channels):
        ...
        self.conv = layers.ConvBNReLU(
            input_channels, output_channels, kernel_size=5, padding=2, bias_attr=False
        )
...
class Decoder(nn.Layer):
    def __init__(self, input_channels, output_channels=(64, 128, 256, 512)):
        ...
        self.alpha_conv = nn.Conv2D(64, 1, kernel_size=5, padding=2, bias_attr=False)
...
class Refine(nn.Layer):
    def __init__(self):
        super().__init__()
        self.conv1 = layers.ConvBNReLU(4, 64, kernel_size=3, padding=1, bias_attr=False)
