...
class MODNet(nn.Layer):
    ...
    def loss(self, logit_dict, label_dict, loss_func_dict=None):
        ...
        loss_fusion_l1 = self.loss_func_dict["fusion"][0](
            matte, alpha
        ) + 4 * self.loss_func_dict["fusion"][0](matte_boundary, alpha)
        # composition loss
        loss_fusion_comp = self.loss_func_dict["fusion"][1](
            matte * label_dict["img"], alpha * label_dict["img"]
        ) + 4 * self.loss_func_dict["fusion"][1](
        ...
        matte_con_sem = paddle.where(transition_mask, logit_semantic, matte_con_sem)
        if False:
            import cv2
...
class HRBranch(nn.Layer):
    ...
    def __init__(self, hr_channels, enc_channels):
        ...
        self.conv_hr = nn.Sequential(
            ...
            Conv2dIBNormRelu(
                hr_channels, 1, 1, stride=1, padding=0, with_ibn=False, with_relu=False
            ),
        )
...
class SEBlock(nn.Layer):
    ...
    def __init__(self, num_channels, reduction=1):
        ...
        self.conv = nn.Sequential(
            nn.Conv2D(num_channels, int(num_channels // reduction), 1, bias_attr=False),
            nn.ReLU(),
