...
class GuidedCxtAtten(nn.Layer):
    def __init__(
        self, out_channels, guidance_channels, kernel_size=3, stride=1, rate=2
    ):
        super().__init__()
    ...
    def forward(self, img_feat, alpha_feat, unknown=None, softmax_scale=1.0):
        ...
        img_feat = F.interpolate(img_feat, scale_factor=1 / self.rate, mode="nearest")
    ...
    def compute_guided_attention_score(
        self, similarity_map, unknown_ps, scale, self_mask
    ):
        # scale the correlation with predicted scale factor for known and
        # unknown area
        unknown_scale, known_scale = scale[0]
        out = similarity_map * (
            unknown_scale * paddle.greater_than(unknown_ps, paddle.to_tensor([0.0]))
            + known_scale * paddle.less_equal(unknown_ps, paddle.to_tensor([0.0]))
        )
    ...
    def propagate_alpha_feature(self, gca_score, alpha_ps):
        ...
        if self.rate == 1:
            ...
            out = (
                F.conv2d_transpose(gca_score, alpha_ps, stride=self.rate, padding=1)
                / 4.0
            )
    ...
    def get_self_correlation_mask(self, img_feat):
        ...
        self_mask = F.one_hot(
            paddle.reshape(paddle.arange(h * w), (h, w)), num_classes=int(h * w)
        )
    ...
    def process_unknown_mask(self, unknown, img_feat, softmax_scale):
        ...
        if unknown is not None:
            ...
            unknown_scale = paddle.clip(paddle.sqrt(unknown_mean / known_mean), 0.1, 10)
            known_scale = paddle.clip(paddle.sqrt(known_mean / unknown_mean), 0.1, 10)
