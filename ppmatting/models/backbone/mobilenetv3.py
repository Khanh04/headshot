...
class MobileNetV3(nn.Layer):
    ...
    def __init__(
        ...
        self.blocks = nn.Sequential(
            *[
                ResidualUnit(
                    in_c=_make_divisible(
                        inplanes * self.scale
                        if i == 0
                        else self.cfg[i - 1][2] * self.scale
                    ),
        ...
        self.feat_channels = [_make_divisible(self.scale * c) for c in out_channels]
...
class ResidualUnit(nn.Layer):
    def __init__(
        self, in_c, mid_c, out_c, filter_size, stride, use_se, act=None, dilation=1
    ):
        super().__init__()
...
class Hardsigmoid(nn.Layer):
    ...
    def forward(self, x):
        return nn.functional.hardsigmoid(x, slope=self.slope, offset=self.offset)
