...
class PPMatting(nn.Layer):
    ...
    def loss(self, logit_dict, label_dict):
        ...
        loss_semantic = self.loss_func_dict["semantic"][0](
            paddle.log(logit_dict["semantic"] + 1e-6), semantic_label.squeeze(1)
        )
        ...
        # fusion_sigmoid loss
        fusion_alpha_loss = loss_fusion_func[0](
            logit_dict["fusion"], label_dict["alpha"]
        )
        # composion loss
        comp_pred = (
            logit_dict["fusion"] * label_dict["fg"]
            + (1 - logit_dict["fusion"]) * label_dict["bg"]
        )
        comp_gt = (
            label_dict["alpha"] * label_dict["fg"]
            + (1 - label_dict["alpha"]) * label_dict["bg"]
        )
        fusion_composition_loss = loss_fusion_func[1](comp_pred, comp_gt)
        # grandient loss
        fusion_grad_loss = loss_fusion_func[2](
            logit_dict["fusion"], label_dict["alpha"]
        )
        # fusion loss
        ...
        loss["all"] = 0.25 * loss_semantic + 0.25 * loss_detail + 0.25 * loss_fusion
...
class SCB(nn.Layer):
    def __init__(self, in_channels):
        ...
        scb_list += [
            nn.Sequential(
                ...
                nn.Conv2D(self.mid_channels[-1], self.out_channels[-1], 3, padding=1),
            )
        ]
    ...
    def _make_stage(
        self, in_channels, mid_channels, out_channels, padding=1, dilation=1
    ):
        layer_list = [
            layers.ConvBNReLU(in_channels, mid_channels, 3, padding=1),
            layers.ConvBNReLU(
                mid_channels, mid_channels, 3, padding=padding, dilation=dilation
            ),
            layers.ConvBNReLU(
                mid_channels, out_channels, 3, padding=padding, dilation=dilation
            ),
...
class HRDB(nn.Layer):
    ...
    def __init__(self, in_channels, scb_channels, gf_index=(0, 2, 4)):
        ...
        channels = [64, 32, 16, 8]
        self.res_list = [
            resnet_vd.BasicBlock(in_channels, channels[0], stride=1, shortcut=False)
        ]
        self.res_list += [resnet_vd.BasicBlock(i, i, stride=1) for i in channels[1:-1]]
        ...
        self.gates = nn.LayerList([GatedSpatailConv2d(i, i) for i in channels[1:]])
...
class GatedSpatailConv2d(nn.Layer):
    def __init__(
        ...
        self._gate_conv = nn.Sequential(
            ...
            nn.ReLU(),
            nn.Conv2D(in_channels + 1, 1, kernel_size=1),
