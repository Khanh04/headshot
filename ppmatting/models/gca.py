...
class ResNet_D_Dec(nn.Layer):
    def __init__(
        ...
        self.conv1 = nn.utils.spectral_norm(
            nn.Conv2DTranspose(
                self.midplanes, 32, kernel_size=4, stride=2, padding=1, bias_attr=False
            )
        )
        ...
        self.layer4 = self._make_layer(BasicBlock, self.midplanes, layers[3], stride=2)
    ...
    def _make_layer(self, block, planes, blocks, stride=1):
        ...
        layers = [
            block(
                self.inplanes, planes, stride, upsample, norm_layer, self.large_kernel
            )
        ]
...
class ResShortCut_D_Dec(ResNet_D_Dec):
    def __init__(
        self, layers, norm_layer=None, large_kernel=False, late_downsample=False
    ):
        super().__init__(
