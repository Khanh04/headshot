...
class HumanMatting(nn.Layer):
    ...
    def __init__(
        ...
        if if_refine:
            if backbone_scale > 0.5:
                raise ValueError(
                    "Backbone_scale should not be greater than 1/2, but it is {}".format(
                        backbone_scale
                    )
                )
        else:
        ...
        self.decoder5_g = nn.Sequential(
            layers.ConvBNReLU(512 + self.backbone_channels[-1], 512, 3, padding=1),
        ...
        self.decoder4_f = nn.Sequential(
            layers.ConvBNReLU(256 + self.backbone_channels[-2], 256, 3, padding=1),
            layers.ConvBNReLU(256, 256, 3, padding=1),
            layers.ConvBNReLU(256, 128, 3, padding=1),
            nn.Upsample(scale_factor=2, mode="bilinear", align_corners=False),
        )
        ...
        self.decoder3_f = nn.Sequential(
            layers.ConvBNReLU(128 + self.backbone_channels[-3], 128, 3, padding=1),
            layers.ConvBNReLU(128, 128, 3, padding=1),
            layers.ConvBNReLU(128, 64, 3, padding=1),
            nn.Upsample(scale_factor=2, mode="bilinear", align_corners=False),
        )
        ...
        self.decoder2_f = nn.Sequential(
            layers.ConvBNReLU(64 + self.backbone_channels[-4], 128, 3, padding=1),
            layers.ConvBNReLU(128, 128, 3, padding=1),
            layers.ConvBNReLU(128, 64, 3, padding=1),
            nn.Upsample(scale_factor=2, mode="bilinear", align_corners=False),
        )
        ...
        self.decoder1_f = nn.Sequential(
            layers.ConvBNReLU(64 + self.backbone_channels[-5], 64, 3, padding=1),
            layers.ConvBNReLU(64, 64, 3, padding=1),
            layers.ConvBNReLU(64, 64, 3, padding=1),
            nn.Upsample(scale_factor=2, mode="bilinear", align_corners=False),
        )
    ...
    def forward(self, data):
        ...
        src_sm = F.interpolate(
            src, scale_factor=self.backbone_scale, mode="bilinear", align_corners=False
        )
        ...
        err_sm = d0_f[:, 1:2, :, :]
        err_sm = paddle.clip(err_sm, 0.0, 1.0)
    ...
    def loss(self, logit_dict, label_dict, loss_func_dict=None):
        ...
        # refine loss
        if self.if_refine:
            loss_refine = self.loss_func_dict["refine"][0](
                logit_dict["refine"], label_dict["alpha"]
            )
...
class Refiner(nn.Layer):
    ...
    def __init__(self, kernel_size=3):
        ...
        self.conv1 = layers.ConvBNReLU(
            channels[0] + 4 + 3, channels[1], kernel_size, padding=0, bias_attr=False
        )
        self.conv2 = layers.ConvBNReLU(
            channels[1], channels[2], kernel_size, padding=0, bias_attr=False
        )
        self.conv3 = layers.ConvBNReLU(
            channels[2] + 3, channels[3], kernel_size, padding=0, bias_attr=False
        )
    ...
    def forward(self, src, pha, err, hid, tri):
        ...
        x = F.interpolate(
            x, paddle.concat((h_half, w_half)), mode="bilinear", align_corners=False
        )
        y = F.interpolate(
            src, paddle.concat((h_half, w_half)), mode="bilinear", align_corners=False
        )
        ...
        if self.kernel_size == 3:
            ...
            x = F.interpolate(x, paddle.concat((h_full, w_full)), mode="nearest")
            y = src
